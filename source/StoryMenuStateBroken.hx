...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-82, 0).loadGraphic(Paths.image('weekMenuBG'));
		...
		var bgYellow:FlxSprite = new FlxSprite(-200, 56).makeGraphic(FlxG.width, 386, 0xFFF73838);
		bgSprite = new FlxSprite(-200, -340);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			WeekData.setDirectoryFromWeek(WeekData.weeksLoaded.get(WeekData.weeksList[i]));
			var weekThing:MenuItem = new MenuItem(-100, 60, WeekData.weeksList[i]);
		    weekThing.setGraphicSize(Std.int(weekThing.width * 1));
		...
		//tracksSprite.scale.x = 0.07
		//tracksSprite.color = 0xFFe55777;
		//tracksSprite.antialiasing = ClientPrefs.globalAntialiasing;
		//add(tracksSprite);

		var tracks:FlxSprite = new FlxSprite(90, 60).loadGraphic(Paths.image('Menu_Tracks'));
		tracks.setGraphicSize(Std.int(tracks.width * 1));
		tracks.antialiasing = ClientPrefs.globalAntialiasing;
		//tracks.updateHitbox();		
		add(tracks);

		txtTracklist = new FlxText(FlxG.width * 0.05, tracks.y + 60, 0, "", 32);
